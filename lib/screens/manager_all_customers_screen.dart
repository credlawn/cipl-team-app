...
class ManagerAllCustomersScreen extends StatefulWidget {
  final List<Map<String, dynamic>> allEmployees;
  final Map<String, dynamic> summary;

  const ManagerAllCustomersScreen({
    super.key,
  ...
  @override
  State<ManagerAllCustomersScreen> createState() => _ManagerAllCustomersScreenState();
}

class _ManagerAllCustomersScreenState extends State<ManagerAllCustomersScreen> {
  String _filter = 'All'; // 'All', 'Pending', 'Done'
  bool _isSearching = false;
  String _searchQuery = '';
  bool _isLoading = false;
  late List<Map<String, dynamic>> _allEmployees;
  ...
  void initState() {
    super.initState();
    _allEmployees = widget.allEmployees;
  ...
  Widget build(BuildContext context) {
    // Aggregate data from all employees
    List<Map<String, dynamic>> customers = [];
    for (var emp in _allEmployees) {
      final empName = emp['employee_name'] ?? 'Unknown';
      final empCusts = List<Map<String, dynamic>>.from(emp['customers'] ?? []);
      for (var c in empCusts) {
        final custCopy = Map<String, dynamic>.from(c);
        custCopy['handler_name'] = empName;
        customers.add(custCopy);
      }
    }

    final total = _summary['total'] ?? 0;
    final currAct = _summary['current_month']?['activated'] ?? 0;
    final lastAct = _summary['last_month']?['activated'] ?? 0;
    final done = currAct + lastAct;
    final currToday = _summary['current_month']?['today'] ?? 0;
    final lastToday = _summary['last_month']?['today'] ?? 0;
    final today = currToday + lastToday;
    final bal = (total is int ? total : 0) - (done as int);

    // Sort: Pending first, then by decision_date (Oldest first)
    customers.sort((a, b) {
    ...
    if (_filter == 'Pending') {
      ...
      // Filter for records with 38+ days elapsed (only pending ones)
      final now = DateTime.now();
      final today = DateTime(now.year, now.month, now.day);
      
      customers = customers.where((c) {
        if (c['is_activated'] == true) return false;
        final dateStr = c['decision_date']?.toString() ?? '';
        ...
        try {
          ...
          final decisionDate = DateTime(utcDate.year, utcDate.month, utcDate.day);
          final diff = today.difference(decisionDate).inDays;
          return diff >= 38;
        } catch (e) {
          return false;
        }
    ...
    if (_searchQuery.isNotEmpty) {
      ...
      customers = customers.where((c) {
        ...
        final mobile = (c['mobile_no'] ?? '').toString().toLowerCase();
        final arn = (c['arn_no'] ?? '').toString().toLowerCase();
        final handler = (c['handler_name'] ?? '').toString().toLowerCase();
        return name.contains(query) || 
               mobile.contains(query) || 
               arn.contains(query) || 
               handler.contains(query);
      }).toList();
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: _isSearching
            ...
            : const Text('All Customers Summary', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFF1F2937))),
      ...
      body: RefreshIndicator(
        ...
        child: Column(
          children: [
            // Global Summary Card
            ListView( // Use a ListView inside Column or just let the outer RefreshIndicator work
              shrinkWrap: true,
                    ...
                    children: [
                      const Row(
                        ...
                        children: [
                          Text(
                            'Team Wide View',
                            style: TextStyle(fontWeight: FontWeight.w700, fontSize: 18, color: Color(0xFF2563EB)),
                          ),
                        ],
                      ),
                      const SizedBox(height: 16),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 8),
                _buildCompactFilter('Over', _filter == 'Over'),
                const Spacer(),
                Text(
              ...
              child: customers.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final handler = cust['handler_name'] ?? 'Unknown';
                                  ...
                                  Expanded(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Row(
                                          children: [
                                            const Icon(Icons.person_outline, size: 10, color: Color(0xFF3B82F6)),
                                            const SizedBox(width: 4),
                                            Text(
                                              handler,
