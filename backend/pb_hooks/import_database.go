...
func SetupDatabaseHooks(app core.App) {
	...
	app.OnRecordUpdate("database").BindFunc(func(e *core.RecordEvent) error {
		...
		// We detect this by checking if import_job_id is being changed/set during this update.
		originalJobId := e.Record.Original().GetString("import_job_id")
		currentJobId := e.Record.GetString("import_job_id")
		if currentJobId == "" || currentJobId == originalJobId {
			return e.Next()
		}
...
func cleanDatabaseRecord(record *core.Record) {
	...
	// 3. Global Trim for all fields (string/text types only)
	for _, field := range record.Collection().Fields {
		name := field.GetName()
		ftype := field.Type()
		if ftype == "text" || ftype == "url" || ftype == "email" || ftype == "editor" || ftype == "select" {
