...
func forceEmployeeCode(e *core.RecordRequestEvent) error {
	...
	// 1.1 Skip if the user is a Manager or BH (Branch Head)
	// These roles are allowed to update other employees' records.
	role := strings.ToLower(e.Auth.GetString("role"))
	if role == "manager" || role == "bh" || role == "admin" {
		return e.Next()
	}
