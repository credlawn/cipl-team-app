	...
	app.OnServe().BindFunc(func(e *core.ServeEvent) error {
		...
		e.Router.POST("/api/mobile/allocate-leads", func(c *core.RequestEvent) error {
			...
			for _, alloc := range req.Allocations {
				...
				// Track records skipped due to same employee so they can be
				// returned to the pool for other employees to use.
				var sameEmployeeSkipped []string
				...
				for _, dbRecordID := range selectedRecords {
					...
					if existingLead != nil {
						// Same employee → skip and return to pool for other employees
						if existingLead.GetString("employee_code") == alloc.EmployeeCode {
							...
							sameEmployeeSkipped = append(sameEmployeeSkipped, dbRecordID)
							skippedCount++
							continue
						}

						// Different employee → update lead + reallocation history + DB update

						// Step 1: Update existing lead
						// data_code, data_sub_code, custom_code always override from fresh database record
						...
						if historyCollection != nil {
							...
							// Get max allocation_sequence for this database record
							var maxSeq struct {
								Seq int `db:"seq"`
							}
						...
						e.App.DB().NewQuery("UPDATE database SET allocation_count = {:count}, employee_count = {:emp_count}, data_status = 'used' WHERE id = {:id}").
							Bind(dbx.Params{
				...
				// Return same-employee skipped records to the pool.
				// These records were skipped because the lead already belonged to this
				// employee. Another employee in the next iteration can still receive them.
				if len(sameEmployeeSkipped) > 0 {
					e.App.Logger().Info("Returning skipped records to pool", "employee", alloc.EmployeeCode, "count", len(sameEmployeeSkipped))
					availableRecords = append(availableRecords, sameEmployeeSkipped...)
				}
			}
		...
		e.Router.POST("/api/mobile/reallocate-leads", func(c *core.RequestEvent) error {
			...
			for _, alloc := range req.Allocations {
				...
				// How many we want to successfully allocate for this employee
				...
				// employeeAllocated tracks successful allocations for this employee.
				// We iterate ALL eligibleRecords (not just a pre-sliced batch) so that
				// skipped records don't reduce the final allocated count.
				employeeAllocated := 0

				for _, dbRecordID := range eligibleRecords {
					if employeeAllocated >= takeCount {
						break // Reached target for this employee
					}
					dbRecord, err := e.App.FindRecordById("database", dbRecordID)
					if err != nil {
						skippedCount++
