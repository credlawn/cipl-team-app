	...
	for _, dbRec := range dbRecords {
		...
		if dbRec.LeadStatus != "" {
			...
			if !shouldBeInactive {
				...
				if deniedCount.Count > 10 {
					shouldBeInactive = true
				}
			}

			// Condition 3: (CNR OR Denied) status with shuffle_count > 15
			if !shouldBeInactive && (dbRec.LeadStatus == "CNR" || dbRec.LeadStatus == "Denied") && dbRec.ShuffleCount > 15 {
				shouldBeInactive = true
			}
		}
